...
> Every task in Phase 18 declares an `<automated>` verify command (Nyquist-compliant). Wave 0 stubs are created INSIDE the plan-owning tasks (TDD RED step), not as a separate pre-wave. 10 tasks across 5 plans.

| Plan | Task | Wave | Test/Verify Command | Notes |
|------|------|------|---------------------|-------|
| 18-01 | T1 | 1 | `npx vitest run --reporter=basic --no-coverage src/store/__tests__/sort.test.ts && npx tsc --noEmit` + zero-server-import grep on `src/store/sort.ts` | TDD RED first; creates W0 stub `src/store/__tests__/sort.test.ts`; pure helper module (zero MCP/SQLite/Hono imports asserted) |
| 18-02 | T1 | 2 | `npx vitest run --reporter=basic --no-coverage src/store/__tests__/version-repo-sort.test.ts src/store/__tests__/version-repo-cursor.test.ts src/store/__tests__/version-repo.test.ts src/__tests__/tool-budget.test.ts && npx tsc --noEmit` + grep `buildVersionOrderBy\|buildAfterCursorWhere\|encodeVersionCursor` ≥ 3 | TDD RED first; creates W0 stubs `version-repo-sort.test.ts` + `version-repo-cursor.test.ts`; `version-repo.test.ts` migrated for new signature; tool budget gate |
| 18-03 | T1 | 3 | `npx vitest run --reporter=basic --no-coverage src/store/__tests__/hierarchy-repo-sort.test.ts src/tools/__tests__/project-tool.test.ts src/tools/__tests__/sequence-tool.test.ts src/tools/__tests__/shot-tool.test.ts && npx tsc --noEmit` + grep `buildHierarchyOrderBy` ≥ 3 | TDD RED first; creates W0 stub `hierarchy-repo-sort.test.ts`; back-compat regression on existing tool tests |
| 18-03 | T2 | 3 | `npx vitest run --reporter=basic --no-coverage src/__tests__/dashboard-routes-sort.test.ts src/store/__tests__/hierarchy-repo-sort.test.ts src/__tests__/tool-budget.test.ts && npx tsc --noEmit` + grep `TRANSITIONAL` returns 0 + grep `parseVersionSortParam\|parseHierarchySortParam\|parseCursorParam` ≥ 3 | TDD RED first; creates W0 stub `dashboard-routes-sort.test.ts`; **removes 18-02's TRANSITIONAL shim** — verifies grep count is 0 |
| 18-04 | T1 | 2 | `cd packages/dashboard && npx vitest run --reporter=basic --no-coverage src/__tests__/sortHelpers.test.ts && cd /Users/macapple/comfyui-vfx-mcp && npx tsc --noEmit` + zero-server-import grep + grep `DUPLICATE OF` in `lib/sortTypes.ts` ≥ 1 | TDD RED first; creates W0 stub `sortHelpers.test.ts`; `lib/sortTypes.ts` mirror types (architecture-purity); LRU + URL/localStorage state machine |
| 18-04 | T2 | 2 | `cd packages/dashboard && npx vitest run --reporter=basic --no-coverage src/__tests__/SortDropdown.test.tsx && cd /Users/macapple/comfyui-vfx-mcp && npx tsc --noEmit` + grep `role="combobox"\|role="listbox"\|role="option"` ≥ 3 + zero-deps-diff guard | TDD RED first; creates W0 stub `SortDropdown.test.tsx`; WAI-ARIA combobox + listbox + keyboard + focus return; zero new dashboard deps |
| 18-04 | T3 | 2 | `cd packages/dashboard && npx vitest run --reporter=basic --no-coverage src/__tests__/LoadMoreButton.test.tsx && cd /Users/macapple/comfyui-vfx-mcp && npx tsc --noEmit` + grep `role="alert"` ≥ 1 | TDD RED first; creates W0 stub `LoadMoreButton.test.tsx`; standalone component (Open Question #3 RESOLVED — researcher recommendation accepted) |
| 18-05 | T1 | 4 | `cd packages/dashboard && npx vitest run --reporter=basic --no-coverage src/__tests__/api.test.ts` + grep `PaginatedVersionsResponse` ≥ 2 + grep `gridSort\|gridCursor\|gridTotalCount\|gridIsFetching\|gridLoadMoreError` ≥ 5 + grep `treeSort` ≥ 1 | Updates pre-existing `api.test.ts`; signal additions; intentional tsc gate deferred to T2 (Step 6 + acceptance criteria document the bridge) |
| 18-05 | T2 | 4 | `cd packages/dashboard && npx vitest run --reporter=basic --no-coverage src/__tests__/HomeView-sort-defaults.test.tsx src/__tests__/HomeView-sort-toggle.test.tsx src/__tests__/api.test.ts && cd /Users/macapple/comfyui-vfx-mcp && npx tsc --noEmit && npx vitest run --reporter=basic --no-coverage src/__tests__/tool-budget.test.ts` + grep `SortDropdown\|LoadMoreButton` in `HomeView.tsx` ≥ 2 | TDD RED first; creates W0 stubs `HomeView-sort-defaults.test.tsx` + `HomeView-sort-toggle.test.tsx`; closes the tsc gate from T1; orchestration end-to-end |
| 18-05 | T3 | 4 | `npx vitest run --reporter=basic --no-coverage && cd packages/dashboard && npx vitest run --reporter=basic --no-coverage && cd /Users/macapple/comfyui-vfx-mcp && npx tsc --noEmit` + provenance append-only regression grep returns 0 + dashboard zero-server-import regression grep returns 0 | Full-suite phase gate; tool budget at 7-of-12; architecture-purity untouched; append-only provenance preserved |
...
5. **SORT-02 SortDropdown renders 3 grid options + 4 tree options** — grid: Latest / Oldest / Version ↓ (Name A→Z DROPPED — `versions` table has no `name` column; see Plan 18-04 DEVIATION 1); tree: A→Z / Z→A / Newest / Oldest. Engine `SortField` enum still retains all 4 fields for whitelist completeness, but the grid dropdown surfaces only the 3 reachable options. Command: `cd packages/dashboard && npx vitest run src/__tests__/SortDropdown.test.tsx -t "options render"`.
